...
import com.fixiu.scanner.resource.classpath.ClassPathResource;
import com.fixiu.scanner.scanner.Location;
import com.fixiu.scanner.scanner.LocationScannerCache;
import com.fixiu.scanner.scanner.ResourceNameCache;
import com.fixiu.scanner.scanner.classpath.jboss.JBossVFSv2UrlResolver;
import com.fixiu.scanner.scanner.classpath.jboss.JBossVFSv3ClassPathLocationScanner;
...
public class ClassPathScanner<I> implements ResourceAndClassScanner<I> {
    ...
    /**
     * The ClassLoader for loading resources on the classpath.
     */
     ...
     *
     * @param classLoader The ClassLoader for loading resources on the classpath.
     */
    ...
    private Set<String> findResourceNames() {
        ...
        if (!locationResolved) {
            LOG.warn("Unable to resolve location " + location + ". Note this warning will become an error in Scanner.");
        }
    ...
    private List<URL> getLocationUrlsForPath(Location location) {
        ...
        if (classLoader.getClass().getName().startsWith("com.ibm")) {
            ...
            try {
                urls = classLoader.getResources(location.getRootPath() + "/scanner.location");
                if (!urls.hasMoreElements()) {
                    LOG.warn("Unable to resolve location " + location + " (ClassLoader: " + classLoader + ")"
                            + " On WebSphere an empty file named scanner.location must be present on the classpath location for WebSphere to find it!\nNote this warning will become an error in Scanner.");
                }
                while (urls.hasMoreElements()) {
                    URL url = urls.nextElement();
                    locationUrls.add(new URL(UrlUtils.decodeURL(url.toExternalForm()).replace("/scanner.location", "")));
                }
            } catch (IOException e) {
                LOG.warn("Unable to resolve location " + location + " (ClassLoader: " + classLoader + ")"
                        + " On WebSphere an empty file named scanner.location must be present on the classpath location for WebSphere to find it!\nNote this warning will become an error in Scanner.");
